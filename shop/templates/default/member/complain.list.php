    ...
    <tbody>
      ...
      <tr class="bd-line">
        ...
        <td><?php
				...
				?></td>
        <td class="ncm-table-handle"><span><a href="index.php?act=member_complain&op=complain_show&complain_id=<?php echo $val['complain_id'];?>" class="btn-bluejeans"><i class="icon-eye-open"></i>
          <p><?php echo $lang['complain_text_detail'];?></p></a></span>
            <?php if(intval($val['complain_state'])==10) { ?>
          <span><a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['complain_cancel_confirm'];?>','index.php?act=member_complain&op=complain_cancel&complain_id=<?php echo $val['complain_id']; ?>')" class="btn-bittersweet"><i class="icon-ban-circle"></i>
