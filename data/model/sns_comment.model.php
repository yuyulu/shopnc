...
class sns_commentModel extends Model {
    ...
    private function getCondition($condition_array){
        $condition_sql = '';
        if ($condition_array['comment_id'] != ''){
            $condition_sql .= " and comment_id='{$condition_array['comment_id']}' ";
        }
        //ID in
        if($condition_array['comment_id_in'] != '') {
            $condition_sql .= " and comment_id in('{$condition_array['comment_id_in']}')";
        }
        //原帖ID
        if($condition_array['comment_originalid'] != '') {
            $condition_sql .= " and comment_originalid = '{$condition_array['comment_originalid']}'";
        }
        //原帖类型
        if($condition_array['comment_originaltype'] != '') {
            $condition_sql .= " and comment_originaltype = '{$condition_array['comment_originaltype']}'";
        }
        //会员名like
        if($condition_array['comment_membername_like'] != '') {
            $condition_sql .= " and comment_membername like '%{$condition_array['comment_membername_like']}%'";
        }
        //状态
        if($condition_array['comment_state'] != '') {
            $condition_sql .= " and comment_state = '{$condition_array['comment_state']}'";
        }
        //内容
        if($condition_array['comment_content_like'] != '') {
            $condition_sql .= " and comment_content like '%{$condition_array['comment_content_like']}%'";
        }
        //添加时间
        if ($condition_array['stime'] !=''){
            $condition_sql  .= " and comment_addtime >= {$condition_array['stime']}";
        }
        if ($condition_array['etime'] !=''){
            $condition_sql  .= " and comment_addtime <= {$condition_array['etime']}";
        }
