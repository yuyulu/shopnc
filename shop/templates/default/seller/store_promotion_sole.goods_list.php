...
<?php } else { ?>
  ...
  <a class="ncbtn ncbtn-aqua" href="<?php echo urlShop('store_promotion_sole', 'sole_quota_add');?>" title="购买套餐"><i class="icon-money"></i>购买套餐</a>
  <?php } else { ?>
  ...
  <?php } ?>
  <a class="ncbtn ncbtn ncbtn-aqua" href="<?php echo urlShop('store_promotion_sole', 'sole_renew');?>"><i class="icon-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
...
<?php } else {?>
  <strong>套餐过期时间<?php echo $lang['nc_colon'];?></strong> <strong style=" color:#F00;"><?php echo date('Y-m-d H:i:s',$output['sole_quota']['sole_quota_endtime']);?></strong>
<?php }?>
  <ul>
...
<table class="ncsc-default-table">
  ...
  <tbody nctype="choose_goods_list">
    ...
    <tr class="bd-line" data-gid="<?php echo $val['goods_id'];?>">
      ...
      <td class="goods-price"><?php echo $lang['currency'].ncPriceFormat($val['goods_price']);?></td>
      <td class="goods-price"><?php echo $lang['currency'].ncPriceFormat($val['sole_price']);?></td>
      <td class="nscs-table-handle">
        <span><a class="btn-bluejeans" href="javascript:void(0);" nctype="a_choose_goods" data-param="{gid:<?php echo $val['goods_id'];?>}"><i class="icon-edit"></i><p>编辑</p></a></span>
...
$(function(){
    ...
    $('input[nctype="btn_search_goods"]').click(function(){
        var _url = '<?php echo urlShop('store_promotion_sole', 'sole_select_goods');?>';
    ...
    $('div[nctype="div_goods_select"],tr[data-gid]').on('click', 'a[nctype="a_choose_goods"]', function(){
        var _url = '<?php echo urlShop('store_promotion_sole', 'choosed_goods');?>';
    ...
    $('tbody[nctype="choose_goods_list"]').on('click','a[nctype="del_choosed"]', function(){
        $this = $(this);
        var _url = '<?php echo urlShop('store_promotion_sole', 'del_choosed_goods');?>';
        eval('var data_str = ' + $(this).attr('data-param'));
