  ...
  <div class="ncsc-order-info">
    ...
    <div class="ncsc-order-condition">
      ...
      <ul>
        ...
        <li>2. 可以在<a href="index.php?act=store_evaluate&op=list" class="ncbtn-mini">评价管理</a>查看详细内容。</li>
      </ul>
    </div>
    <?php } else { ?>
    <div class="ncsc-order-condition">
    ...
    <table class="ncsc-default-table order">
      ...
      <tbody>
        ...
        <tr class="bd-line">
          ...
          <td><?php echo ncPriceFormat($goods['goods_price']); ?>
            <p class="green">
          ...
          <td class="bdl bdr" rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php echo $output['order_info']['state_desc']; ?>
            ...
            <p><a class="ncbtn" href="index.php?act=store_deliver&op=send&order_id=<?php echo $output['order_info']['order_id']; ?>"/><i class="icon-truck"></i><?php echo $lang['store_order_send'];?></a></p>
            <?php } ?></td>
          <?php } ?>
          <!-- E 合并TD --> 
        </tr>
...
$(function(){
    $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('index.php?act=seller_center&op=get_express&e_code=<?php echo $output['order_info']['express_info']['e_code']?>&shipping_code=<?php echo $output['order_info']['shipping_code']?>&t=<?php echo random(7);?>',function(data){
    		if(data){
    			data = var_send+data;
