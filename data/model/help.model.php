...
class helpModel extends Model{
     ...
     * @return array
     */
    public function getStoreHelpTypeList($condition = array(), $page = '', $limit = 0, $order = 'type_sort asc,type_id desc') {
        $condition['page_show'] = '1';//页面类型:1为店铺,2为会员
        $result = $this->table('help_type')->where($condition)->page($page)->limit($limit)->order($order)->key('type_id')->select();
        return $result;
    }
     ...
     * @return array
     */
    public function getStoreHelpList($condition = array(), $page = '', $order = 'help_sort asc,help_id desc') {
        $condition['page_show'] = '1';//页面类型:1为店铺,2为会员
