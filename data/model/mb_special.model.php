...
class mb_specialModel extends Model{
     ...
     *
     */
    public function getMbSpecialInfoByID($special_id) {
        ...
        $info = $this->table('mb_special')->where($condition)->find();
        return $info;
    }
        ...
        switch ($item_type) {
			...
			case 'goods2':
                ...
                foreach ((array) $item_data['item'] as $value) {
                    $value['goods_image'] = cthumb($value['goods_image'], 360);
                    $new_item[] = $value;
                }
    ...
    public function getMbSpecialModuleList() {
		...
		$module_list['goods2'] = array('name' => 'goods2' , 'desc' => '抢购商品');
        return $module_list;
    }
