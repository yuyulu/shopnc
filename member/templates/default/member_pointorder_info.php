  ...
  <div class="ncm-order-contnet">
    <table class="ncm-default-table order">
      ...
      <tbody>
        ...
        <?php if ($output['order_info']['point_shippingtime'] != '') {?>
        <tr>
          <th colspan="20"> <div class="order-deliver"><span>物流公司： <a target="_blank" href="<?php echo $output['express_info']['e_url'];?>"><?php echo $output['express_info']['e_name'];?></a></span><span> <?php echo $lang['member_pointorder_shipping_code'].$lang['nc_colon'];?> <?php echo $output['order_info']['point_shippingcode'];?> </span><span><a href="javascript:void(0);" id="show_shipping">物流跟踪<i class="icon-angle-down"></i>
              <div class="more"><span class="arrow"></span>
                <ul id="shipping_ul">
                  <li>加载中...</li>
                </ul>
        ...
        <tr>
          ...
          <td class="bdl" rowspan="<?php echo count($output['prod_list']);?>"><?php if ($output['order_info']['point_orderallowcancel']) { ?>
            <p><a href="javascript:void(0)" class="ncbtn ncbtn-bittersweet" onclick="ajax_confirm('<?php echo $lang['member_pointorder_cancel_confirmtip']; ?>','<?php echo MEMBER_SITE_URL;?>/index.php?act=member_pointorder&op=cancel_order&order_id=<?php echo $output['order_info']['point_orderid']; ?>');"><?php echo $lang['member_pointorder_cancel_title']; ?></a></p>
            <?php } ?>
            <?php if ($output['order_info']['point_orderallowreceiving']) { ?>
            <p><a href="javascript:void(0)" class="ncbtn" onclick="ajax_confirm('<?php echo $lang['member_pointorder_confirmreceivingtip']; ?>','<?php echo MEMBER_SITE_URL;?>/index.php?act=member_pointorder&op=receiving_order&order_id=<?php echo $output['order_info']['point_orderid']; ?>');"><?php echo $lang['member_pointorder_confirmreceiving']; ?></a></p>
            <?php } ?></td>
          <?php } ?>
        </tr>
        <?php } }?>
...
$(function(){
    $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('index.php?act=member_pointorder&op=get_express&e_code=<?php echo $output['express_info']['e_code']?>&shipping_code=<?php echo $output['order_info']['point_shippingcode']?>&t=<?php echo random(7);?>',function(data){
    		if(data){
