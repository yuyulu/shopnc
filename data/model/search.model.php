...
//                 showMessage($e->getMessage(),'','html','error');
    ...
    private function _setQueryXS($condition,$order) {
        ...
        if (isset($condition['groupbuy'])) {
            $this->_xs_search->addQueryString('groupbuy'.':'.$condition['groupbuy']);
        }
        if (isset($condition['xianshi'])) {
            $this->_xs_search->addQueryString('xianshi'.':'.$condition['xianshi']);
        }
        ...
        if (isset($condition['is_virtual'])) {
            $this->_xs_search->addQueryString('is_virtual'.':'.$condition['is_virtual']);
        }
        if (isset($condition['price_from']) || isset($condition['price_to'])) {
            $this->_xs_search->addRange('goods_promotion_price',$condition['price_from'],$condition['price_to']);
        }
   ...
   public function getTagCategory($keyword = '') {
        if ($keyword != '') {
            ...
            if (!empty($tag_list) && is_array($tag_list)) {
                foreach($tag_list as $key => $val) {
                    $tag_value = str_replace(',', '==33Hao==', $val['gc_tag_value']);
    ...
    public function indexerSearch($get = array(), $pagesize) {
        ...
        if ($get['price_from']) {
            $condition['price_from'] = $get['price_from'];
        }      
        if ($get['price_to']) {
            $condition['price_to'] = $get['price_to'];
        }
        if ($get['virtual'] == 1) {
