...
<?php if ($output['isOwnShop']) { ?>
  <a class="ncbtn ncbtn-mint"  href="index.php?act=store_promotion_bundling&op=bundling_add" ><i class="icon-plus-sign"></i><?php echo $lang['bundling_add'];?></a>

<?php } else { ?>

  <?php if (empty($output['bundling_quota'])) { ?>
  <a href="javascript:void(0)" class="ncbtn ncbtn-aqua" onclick="go('index.php?act=store_promotion_bundling&op=bundling_quota_add');" title="<?php echo $lang['bundling_quota_add'];?>"><i class="icon-money"></i><?php echo $lang['bundling_quota_add'];?></a>
  ...
  <a class="ncbtn ncbtn-mint"  href="index.php?act=store_promotion_bundling&op=bundling_add" style="right:100px"><i class="icon-plus-sign"></i><?php echo $lang['bundling_add'];?></a>
  <?php }?>
  <a class="ncbtn ncbtn ncbtn-aqua" href="index.php?act=store_promotion_bundling&op=bundling_renew"><i class="icon-money"></i>套餐续费</a>
  <?php } ?>
<?php } ?>
...
<table class="ncsc-default-table">
  <thead>
    ...
    <tr>
      ...
      <td colspan="20"><label for="all" ><?php echo $lang['nc_select_all'];?></label>
        <a href="javascript:void(0);" class="ncbtn-mini" nc_type="batchbutton" uri="index.php?act=store_promotion_bundling&op=drop_bundling" name="bundling_id" confirm="<?php echo $lang['nc_ensure_del'];?>"><i class="icon-trash"></i><?php echo $lang['nc_del'];?></a></td>
    </tr>
  </thead>
  ...
  <tbody>
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle"><span><a href="index.php?act=store_promotion_bundling&op=bundling_add&bundling_id=<?php echo $val['bl_id'];?>" class="btn-bluejeans"><i class="icon-cog"></i>
        <p><?php echo $lang['bundling_edit'];?></p>
        </a></span> <span><a class="btn-grapefruit" href='javascript:void(0);' onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', '<?php echo urlShop('store_promotion_bundling', 'drop_bundling', array('bundling_id' => $val['bl_id']));?>');"><i class="icon-trash"></i>
  ...
  <tfoot>
    <tr>
      ...
      <th colspan="20"><label for="all" ><?php echo $lang['nc_select_all'];?></label>
        <a href="javascript:void(0);" class="ncbtn-mini" nc_type="batchbutton" uri="index.php?act=store_promotion_bundling&op=drop_bundling" name="bundling_id" confirm="<?php echo $lang['nc_ensure_del'];?>"><i class="icon-trash"></i><?php echo $lang['nc_del'];?></a></th>
    </tr>
    <tr>
