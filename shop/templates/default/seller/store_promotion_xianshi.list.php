...
<div class="tabmenu">
  ...
  <a class="ncbtn ncbtn-mint" href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a>
  <?php } else { ?>
  <?php if(!empty($output['current_xianshi_quota'])) { ?>
  <a class="ncbtn ncbtn-mint" style="right:100px" href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a> <a class="ncbtn ncbtn-aqua" href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_quota_add');?>" title=""><i class="icon-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="ncbtn ncbtn-aqua" href="<?php echo urlShop('store_promotion_xianshi', 'xianshi_quota_add');?>" title=""><i class="icon-money"></i><?php echo $lang['xianshi_quota_add'];?></a>
  <?php } ?>
  <?php } ?>
</div>
<?php if ($output['isOwnShop']) { ?>
  ...
  <tbody id="xianshi_list">
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle tr"><?php if($val['editable']) { ?>
        <span> <a href="index.php?act=store_promotion_xianshi&op=xianshi_edit&xianshi_id=<?php echo $val['xianshi_id'];?>" class="btn-bluejeans"> <i class="icon-edit"></i>
        <p><?php echo $lang['nc_edit'];?></p>
        </a> </span>
        <?php } ?>
        <span> <a href="index.php?act=store_promotion_xianshi&op=xianshi_manage&xianshi_id=<?php echo $val['xianshi_id'];?>" class="btn-mint"> <i class="icon-cog"></i>
        <p><?php echo $lang['nc_manage'];?></p>
        </a> </span> <span> <a href="javascript:;" nctype="btn_del_xianshi" data-xianshi-id=<?php echo $val['xianshi_id'];?> class="btn-grapefruit"> <i class="icon-trash"></i>
