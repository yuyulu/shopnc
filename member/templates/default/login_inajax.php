  ...
  <form id="login_form" action="<?php echo urlLogin('login');?>" method="post" class="bg" >
    ...
    <dl>
      ...
      <dd>
        ...
        <img class="fl ml10" src="index.php?act=seccode&op=makecode&nchash=<?php echo getNchash();?>" title="<?php echo $lang['login_index_change_checkcode'];?>" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?act=seccode&op=makecode&nchash=<?php echo getNchash();?>&t=' + Math.random()"><span></span></dd>
    </dl>
    <?php } ?>
    <ul>
    ...
    <div class="enter">
      ...
      <span class="other">
      <?php if (C('qq_isuse') == 1){?>
      <a href="<?php echo MEMBER_SITE_URL;?>/index.php?act=connect_qq" title="QQ账号登录" class="qq"><i></i></a>
      <?php } ?>
      <?php if (C('sina_isuse') == 1){?>
      <a href="<?php echo MEMBER_SITE_URL;?>/index.php?act=connect_sina" title="新浪微博账号登录" class="sina"><i></i></a>
      <?php } ?>
