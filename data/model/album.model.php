...
class albumModel extends Model{
    ...
    public function getClassList($condition,$page=''){
        ...
        $param['field']         = 'album_class.aclass_id,min(album_class.aclass_name) as aclass_name,min(album_class.store_id) as store_id,min(album_class.aclass_des) as aclass_des,min(album_class.aclass_sort) as aclass_sort,min(album_class.aclass_cover) as aclass_cover,min(album_class.upload_time) as upload_time,min(album_class.is_default) as is_default,count(album_pic.aclass_id) as count';
        $param['join_type']     = 'left join';
        $param['join_on']       = array('album_class.aclass_id = album_pic.aclass_id');
        $param['where']         = $this->getCondition($condition);
        $param['order']         = $condition['order'] ? $condition['order'] : 'aclass_sort desc';
    ...
    public function delPic($id, $store_id){
        ...
        if(!empty($pic_list) && is_array($pic_list)){
            ...
            foreach($pic_list as $v){
                if (C('oss.open')) {
                    if ($v['apic_cover'] != '') {
                        oss::del(array(ATTACH_GOODS.DS.$store_id.DS.$v['apic_cover']));
                    }
                } else {
