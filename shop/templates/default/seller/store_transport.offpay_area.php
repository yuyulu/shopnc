...
<?php $areas = $output['areas']; foreach ($areas['children'][0] as $provinceId) { ?>
        <tr>
          ...
          <td>
<?php if (is_array($areas['children'][$provinceId])) { ?>
...
<?php } ?>
      ...
      <tfoot>
        <tr class="tfoot">
          ...
          <a href="javascript:void(0);" class="ncbtn ncbtn-mint" id="submitBtn"><span>提交保存</span></a>
          </div>
          </td>
        </tr>
      </tfoot>
...
$(function(){
    ...
    $('a[nc_type="edit"]').on('click',function(){
            ...
            county_html = '<div id="table_area_box_edit" class="eject_con"><ul class="eject-con-list" city_id="'+$(this).attr('city_id')+'" province_id="'+$(this).attr('province_id')+'">';
            for(i = 0; i < county_array.length; i++){
                county_html += '<li><label><input class="checkbox" type="checkbox"';
                if (typeof(cur_county[county_array[i][0]]) != 'undefined') {
                    county_html += ' checked ' ;
                }
                county_html += (' value="'+county_array[i][0]+'" name="county[]">' + county_array[i][1] + '</label>');
            }
            county_html += '</ul><div class="bottom"><a id="county_submit" class="ncbtn ncbtn-mint" href="javascript:void(0);"><span>确认</span></a> <span style="color:#f30;">确认后，还需要点击页面底部的“提交保存”按钮完成保存操作</span></div></div>';
    ...
    $('body').on('click','#county_submit',function(){
        var cur_td = $('.dialog_content > div > ul');
        var cur_checkbox = cur_td.find('input[type="checkbox"]');
