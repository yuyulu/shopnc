...
.dialog-share .share-content .textarea { line-height: 18px; width: 374px; height: 54px; padding: 4px; float: right; border-color: #CCC #DDD #DDD; resize: none; box-shadow: none;}
  ...
  <form method="post" action="index.php?act=member_snsindex&op=sharestore&<?php if($_GET['irefresh'] == 1){?>irefresh=1<?php }?>" id="sharestore_form">
    ...
    <div class="share-widget">同步分享到：
        <ul class="s-app">
          ...
          <?php }?>
          <?php }?>
          <div class="tip"><span class="arrow"></span>点亮图标分享商品到热门社交网络去~<a target="_blank" href="<?php echo urlMember('member_sharemanage');?>"><?php echo $lang['sharebind_alsosharesetting'];?></a></div>
          <?php }?>
        </ul>
      </div>
    ...
    <div class="share-content">
      ...
      <textarea placeholder="<?php echo $lang['sns_sharestore_contenttip'];?>" class="textarea" name="comment" id="content_ssweibo" resize="none" ></textarea>
      <!-- 店铺列表 start -->
      ...
      <ul id="mycarousel-g" class="snsstorelist jcarousel-skin-tango">
        ...
        <li><a href="javascript:void(0);" value="<?php echo $v['store_id'] ?>"><img title="<?php echo $v['store_name']?>" src="<?php echo getStoreLogo($v['store_avatar']);?>"  width="90px" height="90px"/>
      ...
      <!-- 验证码 -->
      <div id="ss_seccode" class="seccode"><?php echo $lang['nc_checkcode'].$lang['nc_colon']; ?>
    ...
    <div class="bottom">
      ...
      <a href="javascript:DialogManager.close('sharestore');" class="cancel">取消</a> </div>
    <input type="hidden" name="sprivacy" id="sprivacy" value="0"/>
  </form>
</div>
<textarea id="bindtooltip_module" style="display:none;"><div class="bindtooltip"><dl><dt><img src="<?php echo SHOP_TEMPLATES_URL;?>/images/member/shareicon/shareicon_@apikey.png"></dt>
<dd><p><?php echo $lang['sharebind_list_popup_tip1'];?><strong class="ml5 mr5">@apiname</strong><?php echo $lang['sharebind_list_popup_tip2'];?></p><p class="hint"><?php echo $lang['sharebind_list_popup_tip3'];?>@apiname<?php echo $lang['sharebind_list_popup_tip4'];?></p></dd></dl><div class="bottom"><a href="javascript:void(0);" nc_type="finishbtn" data-param='{"apikey":"@apikey"}' class="mini-btn"><?php echo $lang['sharebind_list_finishbind'];?></a><span><?php echo $lang['sharebind_list_unfinishedbind'];?><a target="_blank" href="<?php echo MEMBER_SITE_URL;?>/api.php?act=sharebind&type=@apikey"><?php echo $lang['sharebind_list_againbind'];?></a></span></div></div></textarea>
...
$(function(){
	...
	$.getScript("<?php echo RESOURCE_SITE_URL;?>/js/jcarousel/jquery.jcarousel.min.js", function(){
    	$('#mycarousel-g').jcarousel({visible: 4,itemFallbackDimension: 300});
	});
	//初始化选择的店铺
	...
	$("[nc_type='bindbtn']").bind('click',function(){
	    ...
	    if(isbind == '1'){//已经绑定
		    ...
		    window.open(MEMBER_SITE_URL + '/api.php?act=sharebind&type='+data_str.apikey);
		}
	});
	$("[nc_type='finishbtn']").live('click',function(){
	    ...
	    eval( "data_str = "+data_str);
		//验证是否绑定成功
		var url = '<?php echo urlMember('member_sharemanage', 'checkbind');?>';
