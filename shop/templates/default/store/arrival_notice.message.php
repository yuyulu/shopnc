...
<div class="eject_con">
  <div class="arrival-tip"><i class="icon-ok-circle"></i><?php if ($_GET['type'] == 2) {?>商品预约成功,请关注商品上架时间。<?php } else {?>一旦商品在30日内到货，您将收到邮件或短信通知！<?php }?></div>
</div>
...
<div class="recommend">
  ...
  <ul>
    ...
    <li>
      <dl>
        <dt><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $value['goods_id']));?>" target="_blank" title="<?php echo $value['goods_jingle'];?>"><img src="<?php echo thumb($value, 240);?>" alt="<?php echo $value['goods_name'];?>"/></a><em><?php echo $lang['currency'];?><?php echo ncPriceFormat($value['goods_price']);?></em></dt>
