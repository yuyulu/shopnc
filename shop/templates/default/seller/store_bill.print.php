...
<?php }?>
<tr><td>商家应收</td><td colspan="3"><?php echo ncPriceFormat($output['bill_info']['ob_result_totals']);?></td></tr>
<tr><td>结算明细</td><td colspan="3"></td></tr>
<tr><td colspan="4"><?php echo ncPriceFormat($output['bill_info']['ob_order_totals']);?> (订单金额) - <?php echo ncPriceFormat($output['bill_info']['ob_commis_totals']);?> (佣金) - <?php echo ncPriceFormat($output['bill_info']['ob_order_return_totals']);?> (退单金额) + <?php echo ncPriceFormat($output['bill_info']['ob_commis_return_totals']);?> (退还佣金) - <?php echo ncPriceFormat($output['bill_info']['ob_store_cost_totals']);?> (店铺促销费用)
<?php if (floatval($output['bill_info']['ob_order_book_totals']) > 0) { ?>
+ <?php echo ncPriceFormat($output['bill_info']['ob_order_book_totals']);?> (未退定金)
<?php } ?>
