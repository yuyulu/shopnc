...
//布局换色设置
        ...
        if ($this.parent().hasClass('selected')) {
            $.getJSON('index.php?act=common&op=common_operations', {type : 'del', value : _value}, function(data){
                if (data) {
                    ...
                    $('ul[nctype="quick_link"]').find('a[onclick="openItem(\'' + _value + '\')"]').parent().remove();
                }
            });
        } else {
            ...
            $.getJSON('index.php?act=common&op=common_operations', {type : 'add', value : _value}, function(data){
                if (data) {
                    ...
                    $('ul[nctype="quick_link"]').append('<li><a onclick="openItem(\'' + _value + '\')" href="javascript:void(0);">' + _name + '</a></li>');
                }
            });
        }
    ...
    if ($.cookie('commonPendingMatters') == null) {
        $.ajax({
                url : 'index.php?act=common&op=ajax_pending_matters',
                async : false,
...
function openItem(param) {
    ...
    $('#workspace').attr('src', ADMIN_SITE_URL + '/modules/' + data_str[0] + '/index.php?act=' + data_str[1]);
