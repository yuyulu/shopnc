...
class Db{
	...
	private static function connect($host = 'slave'){
		...
		if (@self::$link[$host] = mysqli_connect($conf['dbhost'].':'.$conf['dbport'], $conf['dbuser'], $conf['dbpwd'])){
			if (!@mysqli_select_db(self::$link[$host],$conf['dbname'])){
			...
			if (!mysqli_query(self::$link[$host],$query_string)){
				throw_exception("Db Error: ".mysqli_error(self::$link[$host]));
			}
		}else {
...
// 	    if (is_object(self::$link[$host]) && !mysqli_ping(self::$link[$host])) {
	        mysqli_close(self::$link[$host]);
	        self::$link[$host] = null;
	    }
	}
	...
	public static function query($sql, $host = 'master'){
		...
		$query = mysqli_query(self::$link[$host],$sql);
    ...
    public static function beginTransaction($host = 'master'){
    	self::connect($host);
    	if (self::$iftransacte){
    		mysqli_query('START TRANSACTION',self::$link[$host]);
    ...
    public static function commit($host = 'master'){
    	if (!self::$iftransacte){
    		$result = mysqli_query('COMMIT',self::$link[$host]);
    ...
    public static function rollback($host = 'master'){
    	if (!self::$iftransacte){
    		$result = mysqli_query('ROLLBACK',self::$link[$host]);
    		self::$iftransacte = true;
    		if (!$result) throw_exception("Db Error: ".mysqli_error(self::$link[$host]));
    	}
    }
