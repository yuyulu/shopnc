  ...
  <div class="fixed-bar">
    <div class="item-title">
      <div class="subject">
        ...
        <h5><?php echo $lang['nc_admin_log_subhead'];?></h5>
      </div>
    </div>
  </div>
  <!-- 操作说明 -->
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=admin_log&op=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?act=admin_log&op=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?act=admin_log&op=get_xml',
        colModel : [
