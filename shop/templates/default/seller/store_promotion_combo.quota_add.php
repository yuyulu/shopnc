...
<div class="ncsc-form-default">
  <form id="add_form" action="<?php if ($output['menu_key'] == 'combo_quota_add') {echo urlShop('store_promotion_combo', 'combo_quota_add');} else {echo urlShop('store_promotion_combo', 'combo_renew');}?>" method="post">
    <input type="hidden" name="form_submit" value="ok" />
    <dl>
      <dt><i class="required">*</i>套餐购买数量：</dt>
      <dd>
        <input id="combo_quota_quantity" name="combo_quota_quantity" type="text" class="text w50"/><em class="add-on">月</em><span></span>
        <p class="hint">购买单位为月(30天)，一次最多购买12个月，购买后立即生效，即可发布推荐组合商品。</p>
        <p class="hint"><?php printf('每月您需要支付%d元。', intval(C('promotion_combo_price')));?></p>
