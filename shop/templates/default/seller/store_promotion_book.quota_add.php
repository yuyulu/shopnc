...
<div class="ncsc-form-default">
  <form id="add_form" action="<?php if ($output['menu_key'] == 'book_quota_add') {echo urlShop('store_promotion_book', 'book_quota_add');} else {echo urlShop('store_promotion_book', 'book_renew');}?>" method="post">
    <input type="hidden" name="form_submit" value="ok" />
    <dl>
      <dt><i class="required">*</i>套餐购买数量：</dt>
      <dd>
        <input id="book_quota_quantity" name="book_quota_quantity" type="text" class="text w50"/><em class="add-on">月</em><span></span>
        <p class="hint">购买单位为月(30天)，一次最多购买12个月，购买后立即生效，即可发布预定商品。</p>
        <p class="hint"><?php printf('每月您需要支付%d元。', intval(C('promotion_book_price')));?></p>
...
$(document).ready(function(){
	...
	$("#add_form").validate({
		...
		submitHandler:function(form){
			var unit_price = parseInt(<?php echo intval(C('promotion_book_price'));?>);
			var quantity = parseInt($("#book_quota_quantity").val());
