  ...
  <form method="post" action="index.php?act=pointvoucher&op=voucherexchange_save" id="exform" onsubmit="ajaxpost('exform', '', '', 'onerror');">
    ...
    <dl>
      <dt>您正在使用<span class="ml5 mr5"><?php echo $output['template_info']['voucher_t_points'];?></span><?php echo$lang['points_unit'];?>&nbsp;兑换&nbsp;1&nbsp;张<br/>
        <?php echo $output['template_info']['store_name'];?><?php echo $output['template_info']['voucher_t_price'].$lang['currency_zh'];?>店铺代金券
       <?php if ($output['template_info']['voucher_t_limit'] > 0){?>
                    （<em>满<?php echo $output['template_info']['voucher_t_limit'];?>减<?php echo $output['template_info']['voucher_t_price'];?></em>）
       <?php } else { ?>
                    （<em>无限额代金券</em>）
       <?php } ?>
        </dt>
    ...
    <div class="button">
      ...
      <a href="javascript:DialogManager.close('vexchange');" class="ncbtn">取消</a>
    </div>
  </form>
  <?php }else {?>
  <div class="errormsg" style="height:50px; text-align:center;"><?php echo $output['message'];?></div>
  <div style="text-align:center;"><a href="javascript:DialogManager.close('vexchange');" class="ncbtn">取消</a></div>
  <?php }?>
</div>
