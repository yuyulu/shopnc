    ...
    <thead>
      ...
      <tr>
        ...
        <label for="all"><?php echo $lang['home_message_select_all'];?></label>
          ...
          <a href="javascript:void(0)" class="ncbtn-mini" uri="<?php echo MEMBER_SITE_URL;?>/index.php?act=member_message&op=dropcommonmsg&drop_type=<?php echo $output['drop_type']; ?>" name="message_id" confirm="<?php echo $lang['home_message_delete_confirm'];?>?" nc_type="batchbutton"><i class="icon-trash"></i><?php echo $lang['home_message_delete'];?></a>
          <?php }?>
          <?php if ($output['drop_type'] == 'msg_system' || $output['drop_type'] == 'msg_seller'){?>
          <a href="javascript:void(0)" class="ncbtn-mini" uri="<?php echo MEMBER_SITE_URL;?>/index.php?act=member_message&op=dropbatchmsg&drop_type=<?php echo $output['drop_type']; ?>" name="message_id" confirm="<?php echo $lang['home_message_delete_confirm'];?>?" nc_type="batchbutton"><?php echo $lang['home_message_delete'];?></a>
          <?php }?>
          <?php }?></td>
      </tr>
    </thead>
    <tbody>
      ...
      <tr class="bd-line">
        ...
        <td class="ncm-table-handle"><?php if ($output['drop_type'] == 'msg_list'){?>
          <span><a href="index.php?act=member_message&op=showmsgcommon&drop_type=<?php echo $output['drop_type']; ?>&message_id=<?php echo $v['message_id']; ?><?php if($v['message_parent_id']>0) echo '#'.$v['message_id']; ?>" class="btn-bluejeans"><i class="icon-edit"></i>
          <p><?php echo $lang['home_message_view_detail'];?></p>
          </a></span> <span><a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['home_message_delete_confirm'];?>?', '<?php echo MEMBER_SITE_URL;?>/index.php?act=member_message&op=dropcommonmsg&drop_type=<?php echo $output['drop_type']; ?>&message_id=<?php echo $v['message_id']; ?>');" class="btn-grapefruit"><i class="icon-trash"></i>
          <p><?php echo $lang['home_message_delete'];?></p>
          </a></span>
          <?php }?>
          <?php if ($output['drop_type'] == 'msg_system' || $output['drop_type'] == 'msg_seller'){?>
          <span><a href="index.php?act=member_message&op=showmsgbatch&drop_type=<?php echo $output['drop_type']; ?>&message_id=<?php echo $v['message_id']; ?><?php if($v['message_parent_id']>0) echo '#'.$v['message_id']; ?>" class="btn-aqua"><i class="icon-eye-open"></i>
          <p><?php echo $lang['home_message_view_detail'];?></p>
          </a></span><span><a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['home_message_delete_confirm'];?>?', '<?php echo MEMBER_SITE_URL;?>/index.php?act=member_message&op=dropbatchmsg&drop_type=<?php echo $output['drop_type']; ?>&message_id=<?php echo $v['message_id']; ?>');" class="btn-grapefruit"><i class="icon-trash"></i>
          <p><?php echo $lang['home_message_delete'];?></p>
          </a></span>
          <?php }?></td>
      </tr>
