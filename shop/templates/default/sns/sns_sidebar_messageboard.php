...
<div class="side-message">
  ...
  <div class="guest-form">
    <form id="send_form" action='<?php echo MEMBER_SITE_URL;?>/index.php?act=member_message&op=savemsg&type=sns_board' method="post">
  ...
  <div nctype="message_list" class="message-list">
    ...
    <dl nctype="dl<?php echo $val['message_id'];?>">
      ...
      <dd data-param="{'msgid':'<?php echo $val['message_id'];?>','fname':'<?php echo $val['from_member_name'];?>'}"><span class="time"><?php echo $val['message_time'];?></span><span class="handle">
        ...
        <a  onclick="ajax_get_confirm('<?php echo $lang['nc_common_op_confirm'];?>', '<?php echo MEMBER_SITE_URL;?>/index.php?act=member_message&op=dropcommonmsg&drop_type=sns_msg&message_id=<?php echo $val['message_id'];?>');" href="javascript:void(0);"><?php echo $lang['nc_delete'];?></a>
        <?php }?>
        </span> </dd>
	...
	$('a[nctype="reply_msg"]').live('click', function(){
		...
		if(!p_dd.next().is('dd[nctyoe="replyform"]')){
			$('<dd nctyoe="replyform" class="re-msg"></dd>')
				.append('<i></i>')
				.append('<form id="replyform'+data.msgid+'" action="<?php echo MEMBER_SITE_URL;?>/index.php?act=member_message&op=savereply" method="post"></form>').children('form')
...
function leaveMsgSuccess(data){
	$('<dl></dl>')
		...
		.append('<dd data-param="{\'msgid\':\''+data.msg_id+'\',\'fname\':\''+data.from_member_name+'\'}"><span class="time"><?php echo $lang['sns_just'];?></span><span class="handle"><a href="javascript:void(0);"onclick="ajax_get_confirm(\'<?php echo $lang['nc_common_op_confirm'];?>\', \'<?php echo MEMBER_SITE_URL;?>/index.php?act=member_message&op=dropcommonmsg&drop_type=sns_msg&message_id='+data.msg_id+'\');"><?php echo $lang['nc_delete'];?></a></span></dd>')
