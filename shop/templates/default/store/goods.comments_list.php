        ...
        <div class="ncs-goods-info-content bd" id="ncGoodsRate">
          <div class="top">
			...
			<span>您可对已购商品进行评价</span>
			              <p>
				      <a href="<?php if ($output['goods']['is_virtual']) { echo urlShop('member_vr_order', 'index');} else { echo urlShop('member_order', 'index');}?>" 
				      class="ncbtn ncbtn-grapefruit" target="_blank"><i class="icon-comment-alt"></i>评价商品</a>
				      </p>
            </div>
          </div>
          <!-- 商品评价内容部分 -->
          ...
          <div id="goodseval" class="ncs-commend-main">
            ...
            <div id="t" class="ncs-commend-floor">
			  ...
			  <a <?php if($v['geval_isanonymous'] != 1){?>href="index.php?act=member_snshome&mid=<?php echo $v['geval_frommemberid'];?>" target="_blank" 
			  data-param="{'id':<?php echo $v['geval_frommemberid'];?>}" nctype="mcard"<?php }?>><img src="<?php echo getMemberAvatarForID($v['geval_frommemberid']);?>" ></a>
			  </div>
              <dl class="detail">
	                      <dt>
        ...
        <div class="content">
          <dl class="ncs-comment-goods">
            ...
            <dd class="goods-pic"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $output['goods']['goods_id']));?>"> <img src="<?php echo cthumb($output['goods']['goods_image'], 240); ?>" alt="<?php echo $output['goods']['goods_name']; ?>"> </a> </dd>
                        <!--<dd class="goods-price"><?php // echo $lang['goods_index_goods_price'].$lang['nc_colon'];?><em class="saleP"><?php // echo $lang['currency'].$output['goods']['goods_price'];?></em>-->
