...
class sns_tracelogModel extends Model {
    ...
    private function getCondition($condition_array){
        ...
        if($condition_array['trace_id'] != '') {
            $condition_sql .= " and trace_id = '{$condition_array['trace_id']}' ";
        }
        //自增IDin
        if($condition_array['traceid_in'] != '') {
            $condition_sql .= " and trace_id in('{$condition_array['traceid_in']}') ";
        }
        //原帖ID
        if($condition_array['trace_originalid'] != '') {
            $condition_sql .= " and trace_originalid = '{$condition_array['trace_originalid']}' ";
        }
        //原帖IDin
        if($condition_array['trace_originalid_in'] != '') {
            $condition_sql .= " and trace_originalid in('{$condition_array['trace_originalid_in']}')";
        }
        //会员编号
        if($condition_array['trace_memberid'] != '') {
            $condition_sql .= " and trace_memberid = '{$condition_array['trace_memberid']}' ";
        }
        //会员名like
        if($condition_array['trace_membernamelike'] != '') {
            $condition_sql .= " and trace_membername like '%{$condition_array['trace_membernamelike']}%' ";
        }
        //查看状态
        if ($condition_array['trace_state'] != ''){
            $condition_sql .= " and trace_state = '{$condition_array['trace_state']}' ";
        }
        //允许查看的动态
        if($condition_array['allowshow'] != '') {
            ...
            if ($condition_array['allowshow_memberid'] !=''){
                $allowshowsql_arr[0] = " (trace_memberid = '{$condition_array['allowshow_memberid']}')";
            }
            //查看我关注的人权限为所有人可见的动态
            if ($condition_array['allowshow_followerin'] !=''){
                $allowshowsql_arr[1] .= " (trace_privacy=0 and trace_memberid in('{$condition_array['allowshow_followerin']}'))";
            }
            //查看好友的权限为好友可见的动态
            if ($condition_array['allowshow_friendin'] !=''){
                $allowshowsql_arr[2] .= " (trace_privacy=1 and trace_memberid in('{$condition_array['allowshow_friendin']}'))";
            }
            $condition_sql .=" and (".implode(' or ',$allowshowsql_arr).")";
        }
        //隐私权限
        if ($condition_array['trace_privacyin'] !=''){
            $condition_sql  .= " and trace_privacy in('{$condition_array['trace_privacyin']}')";
        }
        //添加时间
        if ($condition_array['stime'] !=''){
            $condition_sql  .= " and trace_addtime >= {$condition_array['stime']}";
        }
        if ($condition_array['etime'] !=''){
            $condition_sql  .= " and trace_addtime <= {$condition_array['etime']}";
        }
        //内容或者标题
        if ($condition_array['trace_contentortitle'] !=''){
            $condition_sql  .= " and (trace_title like '%{$condition_array['trace_contentortitle']}%' or trace_content like '%{$condition_array['trace_contentortitle']}%') ";
        }
