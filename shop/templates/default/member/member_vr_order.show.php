...
<div class="ncm-oredr-show">
  <div class="ncm-order-info">
      ...
      <div class="content">
        <dl>
          ...
          <dd><?php echo $output['order_info']['buyer_phone'];?>
            ...
            <a href="javascript:void(0);" class="ncbtn-mini ncbtn-bittersweet" dialog_id="vr_code_resend" dialog_title="发送电子兑换码" dialog_width="480" nc_type="dialog" uri="<?php echo urlShop('member_vr_order', 'resend',array('buyer_phone'=>$output['order_info']['buyer_phone'],'order_id'=>$output['order_info']['order_id']));?>"><i class="icon-mobile-phone"></i>重新发送</a>
            <?php } ?>
          </dd>
        </dl>
        <dl class="line">
        ...
        <dl class="line">
          <dt>商&#12288;&#12288;家：</dt>
          <dd><?php echo $output['order_info']['store_name'];?><a href="javascript:void(0)" class="ncbtn" nc_type="dialog" dialog_width="800" dialog_title="查看地图" dialog_id="show_map" uri="index.php?act=show_map&op=index&w=440&h=400&store_id=<?php echo $output['order_info']['store_id'];?>"><i class="icon-map-marker"></i>商家所在信息</a></dd>
        </dl>
      </div>
    </div>
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        <li>1. 您尚未对该订单进行支付，请<a href="index.php?act=buy_virtual&op=pay&order_id=<?php echo $output['order_info']['order_id']; ?>" class="ncbtn-mini ncbtn-bittersweet"><i></i>支付订单</a>以确保及时获取电子兑换码。</li>
        <li>2. 如果您不想购买此订单，请点击 <a href="#order-list" class="ncbtn-mini" >取消订单</a>。 </li>
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        ...
        <li>2. 去商城看看有没有新的<a href="<?php echo urlShop('search','index');?>" class="ncbtn-mini ncbtn-mint" target="_blank">虚拟商品</a>。</li>
      </ul>
    </div>
    <?php }?>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="tl"><dl class="goods-name">
              <dt><a href="<?php echo urlShop('goods','index',array('goods_id'=>$output['order_info']['goods_id']));?>" target="_blank" title="<?php echo $output['order_info']['goods_name'];?>"><?php echo $output['order_info']['goods_name'];?></a><span class="rec"><a target="_blank" href="<?php echo urlShop('vr_snapshot', 'index', array('order_id' => $output['order_info']['order_id']));?>">[交易快照]</a></span></dt>
              <?php if ($output['order_info']['goods_spec']) { ?>
              <dd><span><?php echo $output['order_info']['goods_spec'];?></span></dd>
              <?php } ?>
              ...
              <!-- S消费者保障服务 -->
              <dd>
                <?php if($output['order_info']["contractlist"]){?>
                  <?php foreach($output['order_info']["contractlist"] as $gcitem_v){?>
          ...
          <td class="tl"><?php echo ncPriceFormat($output['order_info']['goods_price']);?></td>
          ...
          <?php if ($output['order_info']['if_cancel']){ ?>
            ...
            <p><a class="ncbtn ncbtn-aqua" href="index.php?act=member_evaluate&op=add_vr&order_id=<?php echo $output['order_info']['order_id']; ?>"><?php echo $lang['member_order_want_evaluate'];?></a></p>
            <?php } ?>
...
//兑换码列表过多时出现滚条
$(function(){
	$('#codeList').perfectScrollbar({suppressScrollX:true});
	//title提示
    	$('.tip').poshytip({
