    ...
    <table class="flex-table">
      ...
      <tbody>
        ...
        <tr data-id="<?php echo $val['class_id']; ?>">
          ...
          <td class="handle-s"><a href="index.php?act=cms_article_class&op=cms_article_class_drop&class_id=<?php echo $val['class_id'];?>" class="btn red confirm-del"><i class="fa fa-trash-o"></i><?php echo $lang['nc_del'];?></a></td>
	...
	$('.flex-table').flexigrid({
        ...
        buttons : [
				   ...
				   {display: '<i class="fa fa-trash"></i>批量删除', name : 'del', bclass : 'del', title : '将选定行数据批量删除', onpress :  function() {
                    ...
                    location.href = 'index.php?act=cms_article_class&op=cms_article_class_drop&class_id=__IDS__'.replace('__IDS__', ids.join(','));
                }
                }
               ]
		});
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?act=cms_article_class&op=cms_article_class_add';
    }
}
</script>