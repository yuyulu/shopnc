...
class p_boothModel extends Model {
    ...
    public function getBoothQuotaInfoCurrent($store_id) {
        ...
        $condition['booth_state'] = self::STATE1;
    ...
    public function addBoothQuota($insert, $replace = false) {
        return $this->table('p_booth_quota')->pk(array('booth_quota_id'))->insert($insert, $replace);
    }
     ...
     * 获取推荐展位商品ID随机列表
     */
    public function getBoothGoodsIdRandList($gcIds, $exceptGoodsId = 0, $limit = false)
    {
        $where = array(
            'booth_state' => self::STATE1,
            'gc_id' => array('in', (array) $gcIds),
        );

        if ($exceptGoodsId > 0) {
            $where['goods_id'] = array('neq', $exceptGoodsId);
        }

        $data = (array) $this->table('p_booth_goods')
            ->field('goods_id')
            ->where($where)
            ->order('sin(goods_id * ' . time() . ')')
            ->limit($limit)
            ->select();

        return Tpl::uniqueValues($data, 'goods_id');
    }
